...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        toast.error(result.error.message ?? "Echec de la creation du compte")
      } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4 pt-4">
                <Button
                  type="submit"
                  disabled={loading}
                  className="relative w-full h-16 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-black uppercase tracking-widest text-xs shadow-xl shadow-orange-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] group disabled:opacity-60"
                >
                  {loading ? (
