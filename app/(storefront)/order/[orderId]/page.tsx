        ...
        <section className="pt-24 pb-20 px-6 md:px-12 bg-[#0D5C3F] rounded-b-[4rem] md:rounded-b-[8rem]">
          <div className="max-w-7xl mx-auto text-center">
            <h1 className="text-6xl md:text-8xl font-black text-white tracking-tighter leading-none italic">
              Commande <span className="text-orange-500 not-italic">introuvable</span>
            </h1>
          </div>
        </section>
      ...
      <section className="pt-24 pb-20 px-6 md:px-12 bg-[#0D5C3F] relative overflow-hidden rounded-b-[4rem] md:rounded-b-[8rem]">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter leading-none mb-6 italic">
            Commande <span className="text-orange-500 not-italic">confirmée</span>
          </h1>
          ...
          <div className="rounded-[2rem] bg-white border border-zinc-100 shadow-2xl shadow-black/[0.04] p-8">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex justify-between items-center">
                <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Type</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Statut</span>
                <span className="font-bold text-zinc-800">{getStatusLabel(order.status)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Client</span>
                <span className="font-bold text-zinc-800">{order.customerInfo.name}</span>
              </div>
              {order.customerInfo.email && (
                <div className="flex justify-between items-center">
                  <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Email</span>
                  <span className="font-medium text-zinc-600">{order.customerInfo.email}</span>
                </div>
              )}
              {order.deliveryAddress && (
                <div className="flex justify-between items-start">
                  <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Adresse</span>
          ...
          <div className="rounded-[2rem] bg-white border border-zinc-100 shadow-2xl shadow-black/[0.04] p-8">
              ...
              {order.items.map((item: any, i: number) => (
                <div key={i} className="flex justify-between text-sm">
                  <div>
                    ...
                    <span className="text-zinc-400 font-bold ml-2">x{item.quantity}</span>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Sous-total</span>
              ...
              {order.deliveryFee !== undefined && order.deliveryFee > 0 && (
                <div className="flex justify-between">
                  <span className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Livraison</span>
